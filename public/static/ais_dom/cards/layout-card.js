customElements.whenDefined("card-tools").then(() => {
  ...
  class LayoutCard extends cardTools.LitElement {
    async setConfig(config) {
      ...
      window.addEventListener("hass-open-menu", () =>
        setTimeout(() => this.build(), 100)
      );
      window.addEventListener("hass-close-menu", () =>
        setTimeout(() => this.build(), 100)
      );
      window.addEventListener("location-changed", () => {
        if (location.hash === "") setTimeout(() => this.build(), 100);
      });
      if (config.rebuild) window.setTimeout(() => this.build(), config.rebuild);
    }
    ...
    firstUpdated() {
      ...
      if (this.rtl)
        this.shadowRoot.querySelector("#columns").style.flexDirection =
          "row-reverse";
      this.build();
      this._cardModder = {
        target: this,
    ...
    update_columns() {
      const width =
        (this.shadowRoot &&
          this.shadowRoot.querySelector("#columns").clientWidth) ||
        (this.parentElement && this.parentElement.clientWidth);
    ...
    build() {
      ...
      const shortestCol = () => {
        ...
        for (let j = 0; j < this.colNum; j++) {
          if (colSize[j] < this.min_height) return j;
          if (colSize[j] < colSize[i]) i = j;
        }
        return i;
      };
