((LitElement, ButtonBase) => {
  ...
  class ButtonCard extends LitElement {
    ...
    getFontColorBasedOnBackgroundColor(backgroundColor) {
      ...
      const parsedBackgroundColor = parsedRgbColor
        ? parsedRgbColor
        : this.hexToRgb(backgroundColor.substring(1));
      let fontColor = ""; // don't override by default
      if (parsedBackgroundColor) {
        // Counting the perceptive luminance - human eye favors green color...
        const luminance =
          (0.299 * parsedBackgroundColor[1] +
    ...
    buildCssColorAttribute(state, config) {
      ...
      if (state) {
        let configState = config.state
          ? config.state.find((configState) => {
    ...
    blankCardColoredHtml(state, config) {
      ...
      return html`
        <ha-card
          style="color: ${fontColor}; background-color: ${color}; ${config.card_style}"
          on-tap="${(ev) => this._toggle(state, config)}"
        >
        </ha-card>
      `;
    }
    ...
    cardColoredHtml(state, config) {
      ...
      return html`
        <ha-card
          ...
          @tap="${(ev) => this._toggle(state, config)}"
        >
          <button-card-button
            style="background-color: ${color}; ${config.card_style}"
          >
            <div style="${config.card_style}">
              ${config.icon
                ? html`
          ...
          <button-card-button style="${config.card_style}">
            <div style="${config.card_style}">
              ${config.icon
                ? html`
                    <ha-icon
                      style="color: ${color}; width: ${config.size}; height: ${config.size};"
                      icon="${icon}"
                    ></ha-icon>
                  `
                : ""}
    ...
    setConfig(config) {
      ...
      this.config.color_off = config.color_off
        ? config.color_off
        : "var(--disabled-text-color)";
      this.config.default_color = config.default_color
        ? config.default_color
        : "var(--primary-text-color)";
