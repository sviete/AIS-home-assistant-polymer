class EntityAttributesCard extends HTMLElement {
      ...
      Object.keys(hass.states)
        ...
        .forEach((key) => {
          Object.keys(hass.states[key].attributes)
            ...
            .forEach((attr_key) => {
              if (
                filters.every((filterFunc) => filterFunc(`${key}.${attr_key}`))
              ) {
                attributes.set(`${key}.${attr_key}`, {
                  name: `${
                    filter.name ? filter.name : attr_key.replace(/_/g, " ")
                  }`,
                  value: `${
                    hass.states[key].attributes[attr_key]
                  } ${filter.unit || ""}`.trim(),
                });
              }
            });
  ...
  set hass(hass) {
    ...
    if (config.filter.exclude) {
      const excludeAttributes = this._getAttributes(
        hass,
