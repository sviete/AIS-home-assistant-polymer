class MonsterCard extends HTMLElement {
  _getEntities(hass, filters) {
    ...
    function _filterName(stateObj, pattern) {
      let compareEntity = stateObj.attributes.title
    ...
    function _complexCompare(a, b) {
      ...
      let y = b;
      let x = a;
      if (!isNaN(a) && typeof b == "string" && b.split(" ").length > 1) {
    ...
    filters.forEach((filter) => {
      ...
      if (filter.domain) {
        filters.push(
          (stateObj) => stateObj.entity_id.split(".", 1)[0] === filter.domain
        );
      }
      if (filter.attributes) {
        Object.keys(filter.attributes).forEach((key) => {
          filters.push((stateObj) =>
            _complexCompare(stateObj.attributes[key], filter.attributes[key])
          );
        });
      }
      if (filter.entity_id) {
      ...
      if (filter.state) {
        filters.push((stateObj) =>
          _complexCompare(stateObj.state, filter.state)
        );
      }
      ...
      Object.keys(hass.states)
        ...
        .forEach((key) => {
          if (filters.every((filterFunc) => filterFunc(hass.states[key]))) {
            entities.set(
              hass.states[key].entity_id,
              Object.assign({ entity: hass.states[key].entity_id }, options)
            );
          }
        });
    });
  ...
  set hass(hass) {
    ...
    if (config.filter.exclude) {
      const excludeEntities = this._getEntities(
        hass,
        config.filter.exclude
      ).map((entity) => entity.entity);
      entities = entities.filter(
        (entity) => !excludeEntities.includes(entity.entity)
      );
    }
    ...
    if (entities.length === 0 && config.show_empty === false) {
      ...
      if (
        (config.when &&
          hass.states[config.when.entity].state == config.when.state) ||
        !config.when
      ) {
    ...
    if (
      ...
      config.card.entities.length !== entities.length ||
      !config.card.entities.every(
        (value, index) => value.entity === entities[index].entity
      )
    ) {
