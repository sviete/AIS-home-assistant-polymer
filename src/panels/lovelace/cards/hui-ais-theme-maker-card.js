customElements.whenDefined("card-tools").then(() => {
  class ThemeMaker extends cardTools.LitElement {
    ...
    updateStyles() {
      ...
      Object.keys(this.theme).forEach((style) => {
        document
          .querySelector("html")
          .style.setProperty("--" + style, this.theme[style]);
      });
    }
    ...
    styleChanged(ev) {
      ...
      if (property) this.theme[property] = value;
      ...
      if (cmd === "NEW")
        this.updateComplete.then(() => {
          this.shadowRoot
            .querySelector("ha-card #classes div #VAL-" + property)
            .focus();
        });
    }
    ...
    importTheme(yaml) {
      ...
      this.name =
        lines[0].trim().substr(0, lines[0].trim().length - 1) || "new_theme";
    ...
    yamlMode(ev) {
      ...
      `;
      card.querySelector("mwc-button").onclick = () => {
        this.importTheme(card.querySelector("textarea").value);
        cardTools.closePopUp();
      };
      card.querySelector("textarea").value = this.exportTheme();
      const popup = cardTools.popUp(
        "Theme",
  ...
  customElements.define("hui-ais-theme-maker-card", ThemeMaker);
});

window.setTimeout(() => {
  if (customElements.get("card-tools")) return;
  customElements.define(
    "hui-ais-theme-maker-card",
